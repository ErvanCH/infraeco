...
# gui <- unique(gui[,-c("taxon_orig","code_habitat")])
# #View(gui[order(v_taxon_id)])
# 
# # Remove duplicates
# gui[,index:=rowSums(.SD), .SDcols = 2:3]
# A <- gui[duplicated(v_taxon_id)]
# AA <- gui[,.I[duplicated(gui[,.(v_taxon_id,guild_fr)][order(v_taxon_id,index)])]]
# gui2 <- gui[order(v_taxon_id,index)][-AA,]
# head(gui2[v_taxon_id%in%v_taxon_id[duplicated(v_taxon_id)]][order(v_taxon_id,index)])
# table(gui2[,duplicated(v_taxon_id),by=guild_fr]$V1)  # pas de duplicat par milieu

# # Import obs (pour guilde de Helder - outdated)
# obs <- setDT(read.csv("data/data_11072019.csv"))
# 
# link_guild <- setDT(read.csv("data/taxa_in_only_one_guilde_11072019.csv"))

# # Import obs (pour guilde de Stefan - 18/07/2019)
obs <- setDT(read.csv("data/extraction_Leitarten_17072019.csv"))

guild <- setDT(read.csv2("data/Leitarten_3Probegilden.csv"))
...
### Create table guilds (outdated, requête Helder solve this issue)
#######################
...
# gui3 <- within(gui3,code_prio[code_prio=="K"] <- NA)
# 
# # Export table d'espèces
# gui4 <- gui3[,c("v_taxon_id", "old_id","name","original_name","characteristic", "dominant", "indigenous", "code_prio", "guild_fr", "guild_de")]
# names(gui4) <- c("id_taxon","old_id","name","name_spp","characteristic","dominant","indigenous","code_prio","guild_fr","guild_de")
# write.csv2(gui4,file="data/guilde_08.07.2019.txt",row.names=F)

# Swiss coordinates
#obs2 <- obs2 %>% filter(!is.na(x_swiss) & milieu_fr=="prairies humides eutrophes" & carac==1) 
...
# limit <- st_read("Z:/GIS/Grenzen/ch_aussengrenze.shp")
# reg <- st_read("Z:/GIS/Regionen/biogeoch_11_lr2002.shp")



#create grid of 1 ha
# grid <- limit[1] %>% st_make_grid(cellsize = 100, what = "polygons")
# # st_write(grid, "data/shp/grid1ha.shp")
# grid_sf <- st_sf(id = 1:length(st_geometry(grid)),geometry = st_geometry(grid))
# class(grid_sf)
# st_crs(grid_sf)
# save(grid_sf,file="data/grid_1ha.Rdata")
load("data/grid_1ha.Rdata")
...
# st_crs(obs_sf) <- st_crs(grid_sf)
# st_crs(riv) <- st_crs(grid_sf)

# Keep only cells (with precision <= 100m) that are in river buffer
buf <- st_read("C:/Dossier_Ervan/SIG/G2-Eaux.dynamiques/Export_shp/Buffer_river/buffer_500m.shp")
st_crs(obs_sf) <- st_crs(buf)
system.time(test <- st_join(obs_sf[obs_sf$v_xy_radius<=100,], buf[,"OBJECTID_G"], join = st_intersects)) # 12 sec
obs_sf <- test[!is.na(test$OBJECTID_G),]
obs_sf2 <- obs3 %>%  st_as_sf(coords = c("x_swiss","y_swiss"))
rm(list=c("test","obs","obs2","obs3","guild","buf"))
...
dist2river <-  function(x) {
  GEOM <- st_geometry(x[[24]])
  RIV <- riv[riv$OBJECTID_G%in%x["OBJECTID_G"],] 
  st_crs(GEOM) <- st_crs(RIV)
  dist <- min(st_distance(GEOM,RIV))
  dist
}

system.time(dist <- parApply(cl,obs_sf, 1, function(x) dist2river(x)))
stopCluster(cl)
...
grid <- as(alt_ra, 'SpatialGridDataFrame')
...
x = read_stars("C:/Dossier_Ervan/R/Ineco_Rproject/data/shp/env100.tif")
