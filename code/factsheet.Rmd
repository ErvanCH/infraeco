...
FF <- list.files(dir)[grep(paste0("_",GUILD,"_spContribution"),list.files(dir))]
RES <- read.csv(paste(dir,FF,sep="/"))
...
if (any(grepl("group",names(RES)))){
  RES <- unique(merge(RES,REF[,c("speciesCODE","lab")],by="speciesCODE",all.x=TRUE))
...
RES2$contribution <- round(RES2$contribution,2)
...
```{r Hist1, eval=T, echo=F,message=F,warning=F,fig.width=SIZE,fig.height=4,fig.pos='H',tidy.opts=list(width.cutoff=50),tidy=TRUE, fig.show='asis', out.width="80%",results='show',fig.cap="<b>Figure 1: Contribution (%) to guild space of each taxon<b>"}
SZ=5
...
# MAX = 3
  ...
  kable(tab,format="html",align=rep('c', 5),booktabs = T,col.names=c("","# ha","% CH"),caption = "<b>Table 3: Number sites (# sites) and corresponding area (area ha). proportion of Swiss territory (4140543 ha, % CH) for  observed (IST) and potential qualities, guild space, and Erg?nzungsbedarf (ha).<b>") %>%
...
temp <- setDT(res)[!is.na(res$CLUST),c("BV.id","CLUST", "subreg","observed_qual", "potential_qual", "EG", "BV_area", "Erganzungsbedarf")]
...
bioreg <- data.frame(bioregion=c(11, 21, 22, 23, 24, 31, 32, 41, 51, 52, 61, 62),name=c("Jura und Randen","Genferseegebiet","Rheingebiet","Oestliches Mittelland","Westliches Mittelland","Voralpen","Nordalpen","Oestliche Zentralalpen ","Westliche Zentralalpen","Engadin","S?dalpen","S?dlicher Tessin"))
