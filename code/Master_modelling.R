...
# LOCATION <- "G://R/"# location of all folders (Ervan's PC)
LOCATION <- "C://Dossier_Ervan/R/"# location of all folders (IF office)

GUILD <- 2

### Select guild
G <- c(1:4,10:13,17,19,22)
...
for (GUILD in G) {
  ...
  guild.info <- guild.info %>% hablar::convert(lgl(RF,GAM,GBM,ME))
  ...
  eg <- LOAD("EG","data",dir)
  EG <-setDT(grid_sf)[grid.id%in%c(IST$grid.id,eg$grid.id),c("grid.id","BV04","canton","subreg","geometry","centro",..VAR1)]
  ...
  rm(list=setdiff(ls(), c("env.pot","OBS","VAR","GUILD","guild.info","dir","DATE")))
